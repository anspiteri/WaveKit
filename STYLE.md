...
* Public Core: Use a `wavekit_` module prefix for public functions and variables to avoid global namespace naming collisions.
	* Example: `wavekit_variable_name`
* Private Core: Use plain snake_case without a module prefix for internal, file-specific names.
* Noun-First Rule: Structure function names as Noun-First, Verb-Second so related actions group together in IDE autocomplete.
	* Example: `wavekit_wav_load()`

## 2. Isolation & Scope Management
* Static Functions: Always mark internal-only functions as *static* to hide them from other files.
* Global Variables: Add a `g_` prefix to all global variables
	* Example: `g_log_callback`
